...
def text_color(output_path):
    ...
    font = ImageFont.truetype("comicz.ttf", size=35)
    y = 100
    # text_for_msg = '''
    # Как смириться
    # с тем, что навсегда
    ...
    text = ' Парадоксальное подкрепление — это когда ты сначала обнадеживаешь человека, а потом лишаешь его надежды. И тут же — снова надежда, снова разочарование. И так далее. Таким образом человека можно привязать к себе. Формула: счастье — смерть — счастье — смерть.'.split(' ')
    arr = [[]]
    count = 0
    ...
    draw.text((20, 20), text_out, '#91e2a8', font)
